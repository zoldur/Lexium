...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='lexium.conf'
CONFIGFOLDER='/root/.lexium'
COIN_DAEMON='lexiumd'
COIN_CLI='lexium-cli'
COIN_PATH='/usr/local/bin/'
COIN_TGZ='https://github.com/zoldur/Lexium/releases/download/v9.3.3/lexium.tar.gz'
...
function create_key() {
  ...
  COINKEY=$($COIN_PATH$COIN_CLI masternode genkey | grep " Secret key " | awk -F '"' '{print $(NF-1)}')
  if [ "$?" -gt "0" ];
    then
